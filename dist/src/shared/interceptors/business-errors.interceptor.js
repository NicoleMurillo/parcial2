...
let BusinessErrorsInterceptor = class BusinessErrorsInterceptor {
    intercept(context, next) {
        return next.handle()
            ...
            if (error.type === errors_1.BusinessError.NOT_FOUND)
                throw new common_1.HttpException(error.message, common_1.HttpStatus.NOT_FOUND);
            else if (error.type === errors_1.BusinessError.PRECONDITION_FAILED)
                throw new common_1.HttpException(error.message, common_1.HttpStatus.PRECONDITION_FAILED);
            else if (error.type === errors_1.BusinessError.BAD_REQUEST)
